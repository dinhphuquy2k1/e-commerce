...
DROP TABLE IF EXISTS shopping_mall_configs;
...
-- tag for shopping mall config
DROP TABLE IF EXISTS shopping_mall_tags;
...
(
    ...
    `updated_at`              TIMESTAMP DEFAULT now() ON UPDATE now(),
    PRIMARY KEY (`id`),
    KEY `shopping_mall_tag_shopping_mall_config_id_foreign` (`shopping_mall_config_id`),
    CONSTRAINT `shopping_mall_tag_shopping_mall_config_id_foreign` FOREIGN KEY (`shopping_mall_config_id`) REFERENCES `shopping_mall_configs` (`id`) ON DELETE CASCADE
);

-- item for tag and small_config
DROP TABLE IF EXISTS shopping_mall_items;
CREATE TABLE shopping_mall_items
(
    `id`                      bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `name`                    VARCHAR(500)        NOT NULL,
    `product_id`              bigint(20) unsigned NULL,
    `shopping_mall_tag_id`    bigint(20) unsigned NULL,
    `shopping_mall_config_id` bigint(20) unsigned NULL,
    `link`                    VARCHAR(500)        NULL COMMENT 'Link for ads',
    `type`                    tinyint(11)         NOT NULL DEFAULT 1 COMMENT '1: link product, 2: link other',
    `created_at`              TIMESTAMP                    DEFAULT now(),
    `updated_at`              TIMESTAMP                    DEFAULT now() ON UPDATE now(),
    PRIMARY KEY (`id`),
    KEY `shopping_mall_item_product_id_foreign` (`product_id`),
    KEY `shopping_mall_item_shopping_mall_tag_id_foreign` (`shopping_mall_tag_id`),
    KEY `shopping_mall_item_shopping_mall_config_id_foreign` (`shopping_mall_config_id`),
    CONSTRAINT `shopping_mall_item_product_id_foreign` FOREIGN KEY (`product_id`) REFERENCES `products` (`id`) ON DELETE CASCADE,
    CONSTRAINT `shopping_mall_item_shopping_mall_tag_id_foreign` FOREIGN KEY (`shopping_mall_tag_id`) REFERENCES `shopping_mall_tags` (`id`) ON DELETE CASCADE,
...
(
    ...
    `type`          tinyint(11)         NOT NULL DEFAULT 1 COMMENT '1: phone, 2: email, 3: address, 4: social',
    `is_use`        tinyint(11)         NOT NULL DEFAULT 1 COMMENT '0: hide, 1: show',
